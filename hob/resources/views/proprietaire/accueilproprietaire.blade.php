...
@media (max-width: 768px) {
  .testimonials-carousel-wrapper { padding: 0 2px; }
  .testimonials-carousel-controls { left: 0; right: 0; }
}

/* Logements Populaires sur Notre Plateforme */
...
<div class="container py-5">
  ...
  <!-- Logements Populaires sur Notre Plateforme -->
  <h2 class="section-title fade-in-left">Logements Populaires sur Notre Plateforme</h2>
      ...
      @forelse($latestLogements as $annonce)
        <div class="logement-card card card-3d h-100 me-3">
          ...
          <div class="card-body" style="padding: 1.2rem 1rem; display: flex; flex-direction: column; gap: 0.5rem;">
            <h3 class="card-title" style="margin-bottom: 0.4rem;">{{ $annonce->titre_anno }}</h3>
            <div class="price" style="margin-bottom: 0.3rem;">{{ $annonce->logement->prix_log ?? 0 }} DH / MOIS</div>
            ...
            <a href="{{ route('proprietaire.details', $annonce->logement->id) }}" class="btn btn-primary mt-2 w-100" style="background: #244F76; border: none; font-weight: 600; margin-top: 0.7rem;">Détails</a>
          </div>
        </div>
      @empty
        <div class="col-12">
  ...
  <div class="row g-0 align-items-center fade-in-up" style="background: linear-gradient(90deg, #c5d6e1 60%, #fff 100%); border-radius:2rem; overflow:hidden;">
    <div class="col-12 col-md-6 p-4 d-flex flex-column align-items-center justify-content-center position-relative" style="min-height:220px;">
      <img src="{{ asset('public/images/map.png') }}" alt="Carte du Maroc" class="w-100 h-100 object-cover rounded-3 shadow" style="max-width:340px; filter:brightness(0.95) blur(0.5px);">
